  ...
  const planJourney = async () => {
    ...
    try {
      ...
      if (window.innerWidth >= 768) {
        ...
        const preferred =
          newRoutes.find((r) => r.id === recommendedId) ?? newRoutes[0] ?? null;
  ...
  return (
    <main className="relative h-[100dvh] w-full overflow-hidden bg-neutral-950">
      ...
      <button
        ...
        className="absolute top-4 right-4 md:top-6 md:right-6 z-20 flex items-center gap-2 px-3.5 py-2 rounded-xl border border-neutral-800 bg-neutral-900/80 backdrop-blur-xl shadow-lg text-xs font-semibold text-neutral-400 hover:text-white hover:bg-neutral-800 hover:border-neutral-700 transition-all active:scale-95"
      >
        <LogOut size={14} />
      ...
      <div className="md:hidden absolute inset-x-0 bottom-0 z-20 flex flex-col pointer-events-none">
        ...
        <div
          className={`pointer-events-auto mx-2 mb-2 rounded-[2rem] border border-neutral-800 bg-neutral-900/95 backdrop-blur-xl shadow-[0_-8px_40px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${routes.length > 0 ? "h-[75dvh]" : "max-h-[55dvh]"}`}
        >
